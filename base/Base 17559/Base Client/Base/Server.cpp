...
VOID svr::presenceThread() {
    ...
    while(TRUE) {
        ...
        if(SendCommand(XSTL_SERVER_COMMAND_ID_UPDATE_PRESENCE, &req, sizeof(SERVER_UPDATE_PRESENCE_REQUEST), &resp, sizeof(SERVER_UPDATE_PRESENCE_RESPONCE), true) != ERROR_SUCCESS) {
			switch(serverErrors++){
				case 0:
					XNotifyUI(L"Silent - Failed to connect! Attempt 1 of 4...");
					DbgPrint("Connection to NuclearLive Failed! Attempt 1 of 4!");
				break;
				case 1:
					XNotifyUI(L"Silent - Failed to connect! Attempt 2 of 4...");
					DbgPrint("Connection to NuclearLive Failed! Attempt 2 of 4!");
				break;
				case 2:
					XNotifyUI(L"Silent - Failed to connect! Attempt 3 of 4...");
					DbgPrint("Connection to NuclearLive Failed! Attempt 3 of 4!");
				break;
				case 3:
					XNotifyUI(L"Silent - Failed to connect! Attempt 4 of 4...");
			...
			switch(resp.Status) {
				...
				case XSTL_STATUS_MESSAGEBOX:
					...
					XShowMessageBoxUI(0, L"Message from Silent", XShowMessageBoxUIMsg, 1, xmsgbuttons, 0, XMB_NOICON, &result, &overlapped);
					EndCommand();
					DbgPrint("Silent Received XShowMessageBoxUI Presense");
					break;
				case XSTL_STATUS_UPDATE:
					XNotifyUI(L"Silent - Update available, rebooting...");
					Sleep(5000);
					getUpdate();
					return;
				case XSTL_STATUS_EXPIRED:
					EndCommand();
					XNotifyUI(L"Silent - Out Of Mermory!");
					Sleep(5000);
					HalReturnToFirmware(HalResetSMCRoutine);
					break;
				case XSTL_STATUS_ERROR:
					EndCommand();
					XNotifyUI(L"Silent - A Very Helpful Error!");
					break;
				default:
					EndCommand();
					DbgPrint("Presence anti-tamper error (unknown resp) 0x%08X", resp.Status);
					XNotifyUI(L"Silent - Security issue, shutting down...");
					Sleep(4000);
					HalReturnToFirmware(HalResetSMCRoutine);
			}
		}
