...
VOID waitForDash(){
	...
	LPCWSTR Buttons[1] = { L"Enter Silent's Domain!."};
	while (XShowMessageBoxUI(XUSER_INDEX_ANY, L"Kernel Can Suck it!", L"\n[17559]\nFree Base : Updated by Silent! , Enjoy the free source on dash 17559! God We luv using Kernels shit!", 1, Buttons, 0, XMB_ALERTICON, &g_mb_result1, &g_xol1) == ERROR_ACCESS_DENIED)
	PatchInJump((PDWORD)0x81740960, (DWORD)XNetXnAddrToMachineIdHookDash, false);
	Sleep(500);
...
HRESULT Initialize() 
{
	...
	else
	{
		...
		BYTE* kv = (BYTE*)malloc(0x4000);//0x00000002000163C0
		QWORD kvAddress = IsDevkit ? HvPeekQWORD(0x00000102000162E0) : HvPeekQWORD(0x00000002000163C0);  //17559
		//QWORD kvAddress = HvPeekQWORD(0x00000002000163C0);
