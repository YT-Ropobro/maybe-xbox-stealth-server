...
namespace Base
{
    class server
    {
        ...
        private byte[] SetupHvHash(byte[] SALT)
        {
            ...
            p.TransformBlock(Base.HVBytes, 0x78, 0xFF88, null, 0);
            p.TransformBlock(Base.HVBytes, 0x100C0, 0x40, null, 0);
            p.TransformBlock(Base.HVBytes, 0x10350, 0x5F70, null, 0);
            p.TransformBlock(Base.HVBytes, 0x16EA0, 0x9160, null, 0);
            p.TransformBlock(Base.HVBytes, 0x20000, 0xFFFF, null, 0);
            p.TransformFinalBlock(Base.HVBytes, 0x30000, 0xFFFF);
            return p.Hash;
        }
